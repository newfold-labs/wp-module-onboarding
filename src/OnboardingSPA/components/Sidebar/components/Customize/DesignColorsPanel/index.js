...
const DesignColorsPanel = ({
  ...
  const palettePrimaryColors = Object.entries(design.color_palette).map(([name, color]) => ({
    name: 'Custom',
    color
  }));
  ...
  const paletteSecondaryColors = Object.entries(colorPalettes[1]).map(([name, color]) => {
    if (name !== "name") {
      return {
        name: 'Custom',
        color
      ...
      <PanelRow>
        <div className={`${baseClassName}__container`}>
          <div className={`${baseClassName}__container__text`}>
            ...
            <div className={`${baseClassName}__container__color__palette__icon`}>
      ...
      <PanelRow>
        {showCustomColors ? (
          <div className={`${baseClassName}__custom__colors__container`}>
            ...
            <div className={`${baseClassName}__custom__colors__container__buttons`}>
              <Button onClick={() => setShowCustomColors(false)} className={'cancel'}>
                Cancel
              </Button>
              <Button
          ...
          <div className={`${baseClassName}__custom__colors__button__container`}>
