...
const SocialMediaForm = ( {
	...
	const checkValidUrl = function ( socialInput, data ) {
		...
		switch ( socialInput ) {
			case SocialMediaSites.TWITTER:
				...
				if (
					isValidTwitterHandle( data ) ||
					isValidTwitterUrl( data )
				) {
		...
		if ( errorResolved ) {
			const activeErrorFiltered = activeError.filter( function ( item ) {
				return item !== socialInput;
			} );
			setActiveError( activeErrorFiltered );
		} else if ( ! activeError.includes( socialInput ) ) {
	...
	const setDataAndActiveErrorState = ( data, socialInput, activeErrors ) => {
		if ( ! data ) {
			const activeErrorFiltered = activeErrors.filter( function ( item ) {
				return item !== socialInput;
			} );
			setActiveError( activeErrorFiltered );
		}

		if ( activeErrors.length === 0 ) setIsValidSocials( true );
		else setIsValidSocials( false );
	};
	...
	const isValidTwitterUrl = ( url ) => {
		return url.match(
			`^http(?:s)?:\/\/(?:www\.)?twitter\.com\/([A-Za-z0-9_]{1,25})\/?$`
		)
			? true
			: false;
	};
