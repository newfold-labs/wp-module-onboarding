							...
							if (!hasBlockGapSupport) {
								...
								combinedSelector =
									selector === ROOT_BLOCK_SELECTOR
										? `:where(.${className}${spacingStyle?.selector || ''})`
										: `:where(${selector}.${className}${spacingStyle?.selector || ''})`;
							} else {
								combinedSelector =
									selector === ROOT_BLOCK_SELECTOR
										? `:root :where(.${className})${spacingStyle?.selector || ''}`
										: `:root :where(${selector}-${className})${spacingStyle?.selector || ''}`;
							}
...
export const getNodesWithStyles = (tree, blockSelectors) => {
	...
	Object.entries(tree.styles?.blocks ?? {}).forEach(([blockName, node]) => {
			...
			Object.entries(node.variations).forEach(
				([variationName, variation]) => {
					...
					Object.entries(variation?.blocks ?? {}).forEach(
						([variationBlockName, variationBlockStyles]) => {
		...
		Object.entries(node?.elements ?? {}).forEach(([elementName, value]) => {
...
export const getNodesWithSettings = (tree, blockSelectors) => {
	...
	// Blocks.
	Object.entries(tree.settings?.blocks ?? {}).forEach(([blockName, node]) => {
