...
export const getChaptersFromTopPriorityAndExperienceLevel = ( initialChapters, topPriority, experienceLevel ) => {
    ...
    switch( experienceLevel ) {
        ...
        case '3':
            finalChapters = filter( initialChapters, ( chapter ) => {
                return ( chapter.id !== topPriorityChapter.id && ( chapter.id === features.id || chapter.id === demographic.id ) )
            } )
            finalChapters = addAfterChapter(
                finalChapters,
                demographic,
