		...
		{ 
			...
			if(sizeof($flow_data_fixes) > 0) {
				foreach ($flow_data_fixes as $index=>$fix) {
					if (array_key_exists($fix['old_key'], $flow_data) && array_key_exists($fix['new_key'], $default_flow_data)) {
						($fix['retain_existing_value'])?
							$flow_data[$fix['new_key']] = $flow_data[$fix['old_key']]
							: $flow_data[$fix['new_key']] = $default_flow_data[$fix['new_key']];
