	...
	public function update_item(\WP_REST_Request $request)
	{
		...
		foreach ($params as $param_key => $param_value) {
			if (!array_key_exists($param_key, $settings)) {
				return new \WP_Error('param_key_not_present', "The provided param key '{$param_key}' does not match", array('status' => 400));
			}
			...
			if (in_array($param_key, $this->social_urls_to_validate)) {
				...
				if (!is_array($param_value)) {
					...
					if (!\wp_http_validate_url($param_value)) {
						return new \WP_Error('param_not_proper_url', "The provided param '{$param_value}' is NOT a proper URL", array('status' => 400));
					}
				} else {
					foreach ($param_value as $param_url) {
						...
						if (!\wp_http_validate_url($param_url)) {
							return new \WP_Error('param_not_proper_url', "The provided param '{$param_url}' is NOT a proper URL", array('status' => 400));
						}
					}
				}
			}
